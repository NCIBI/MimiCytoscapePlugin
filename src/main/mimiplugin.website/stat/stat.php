...
mssql_select_db($db);
$query="select IP, host, visittime, mimipluginVersion from R2.UserStat order by visittime desc";
$results=mssql_query($query)or die("Sorry, \"$query\" failed.");
$len=mssql_num_fields($results);
print "<table border=1><tr><td>IP</td><td>HOST</td><td>VISIT TIME</td><td>MiMIplugin Version</td></tr><tr>";
