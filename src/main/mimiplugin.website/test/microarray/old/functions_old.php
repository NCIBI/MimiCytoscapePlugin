...
<?
   ...
   foreach ($seriesids as $sid){
          ...
          while ($rowSa= mssql_fetch_array($resultSa)){
                 if ($i%6==0){
                     $str.="<td><font size=-2><input type='checkbox' name='selectSamples".$sid."[]' value='".$rowSa[1]."'><a href='searchSample.php?sampleName=".$rowSa[0]."' style='text-decoration:none'>".$rowSa[0]."</a></font></td></tr><tr><td width=94>&nbsp;</td>";
                     $i++;
                 }
                 else { 
                     $str.="<td><font size=-2><input type='checkbox' name='selectSamples".$sid."[]' value='".$rowSa[1]."'><a href='searchSample.php?sampleName=".$rowSa[0]."' style='text-decoration:none'>".$rowSa[0]."</a></font></td>";
                     $i++;
                 }
          }
          ...
          $str.="</select></td></tr><tr><td width='94' valign='top'>Sample(s)</td><td colspan='6'>Click above check box to choose desired SAMPLES<strong> OR </strong><br/><input type='checkbox' name='selectAllsamples".$sid."' onClick='selectAllSamples(this,\"".$sid."\")'>Select All Samples</td></tr><tr><td width='94'>&nbsp;</td><td colspan='6'><input type='submit' name='gobutton' id='gobutton' value='Query for Genes...' onClick='return checkGenes(\"$sid\")'></td></tr><tr><td>&nbsp;</td><input type='hidden' name='seriesID' value='".$sid."'><input type='hidden' name='species' value='".$org."'><input type='hidden' name='seriesName' value='".$seriesName."'><tr><td width='94'><a href='http://developer2.ncibi.org:8595/gp/runTask.jsp?name=ttest.custom&LSID=urn:lsid:8080.twiggin.141.214.17.5:genepatternmodules:8:2&userid=jinggao@umich.edu' >ttest</a></td><td>&nbsp;</td></tr></form>";
   }//foreach loop 
   $str.="</table>";
   print $str;
}
