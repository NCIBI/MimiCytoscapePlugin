...
if (isset($para['command'])){
	...
	else if ((strcmp($command,"lookup") == 0 || strcmp($command,"unilink") == 0)&& !(isset($para['db'])&& ( strcmp($para['db'],"MiMI") == 0 ||strcmp($para['db'],"GIN") == 0 ))){
		$ret=processRequest($para);
        	print $ret;
		}	
        	else if (strcmp($command,"time_image_jpg") == 0){//to do 
		     }
		     else {
			...
			if ( strcmp($para['db'],"MiMI") == 0)
				$query="exec R2.UserQueryStat_VA' $rmtIP','$rmtHost','$ids','$organismID','$moleculeType','$dataSource','$pluginVersion'";
			if (strcmp($para['db'],"GIN") == 0)//if query GIN, set dataresource with "GIN"
				$query="exec R2.UserQueryStat_VA' $rmtIP','$rmtHost','$ids','$organismID','NONE','GIN','$pluginVersion'";
			...
			if (strcmp($para['db'],"GIN") == 0){
				//get gene ids if inputs are gene symbols
				$symbols=formatInputgenes($ids);
				$db="gener2";
			        conn_db_visant();
                                mssql_select_db( $db);
				$query="Select geneid From dbo.Gene Where symbol in ($symbols) and taxid=$organismID";
				$db="pubmed";
