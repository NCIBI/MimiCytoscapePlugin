...
/*$para=$_GET;
print "parameter are:\n";
foreach ($para as $color) {
    print  "[$color]\n";
}*/

/*$TIME_P='proxy';
if (isset($_REQUEST['command'])&& isset($_REQUEST['target'])){
	$commandVal=$_REQUEST['command'];
	$targetUrl=$_REQUEST['target'];
	if (endwith($commandVal,$TIME_P) ) { 
...
if (strcmp($command,"lookup") != 0 && strcmp($command,"unilink") != 0)  {
	$targetUrl=$_REQUEST['target'];
	$ret=readRemote($targetUrl);
	print $ret;
}
